<?php
  ...
  if (isset($_POST['draw'])) {
    $column = ['id', 'name', 'location', 'code', ''];

    $qry = "SELECT id, name, location, code FROM tbl_projects ";
    ...
    foreach($result as $row) {
      ...
      $subArray[] = "
        <div class='d-flex justify-content-center align-items-center'>
          <button class='btn btn-link btn-sm m-0' onclick='handleGetEditProject(" . $row['id'] . ")'>Edit</button> |
          <button class='btn btn-link btn-sm m-0' onclick='handleDeleteProject(" . $row['id'] . ")'>Delete</button>
        </div>
      ";
      $data[] = $subArray;
    }
